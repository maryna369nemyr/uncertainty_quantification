...
def generate_heat_map(N, mean_vec, L, samples, figure_name):
    ...
    plt.imshow(gaussian_RF, cmap = "coolwarm")
...
if __name__== '__main__':
    ...
    for i in range(3):
        ...
        cov_std = np.identity(N ** 2)
        samples = np.random.multivariate_normal(mean_std, cov_std, 1)
        generate_heat_map(N, mean_func, L_cov_exp, samples,  "Exp kernel_" + str(i))
        generate_heat_map(N, mean_func, L_cov_sq_exp, samples, "Squared exp kernel_"+ str(i))
    plt.show()
