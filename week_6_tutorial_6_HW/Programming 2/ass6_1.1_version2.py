...
def compute_barycentric_weights2(grid):
    size    = len(grid)
    w       = np.ones(size)
...
def barycentric_interp2(eval_point, grid, func_eval):
    weights     = compute_barycentric_weights2(grid)
...
def interpolation_over_nodes(sol_odeint_true, grid_N, t_max, x_axis, model, init_cond, params_odeint, atol, rtol):
    ...
    for i, grid_size in enumerate(grid_N):
        ...
        output_interpolation = np.array([barycentric_interp2(x_eval, cheb_grid, func_eval) for x_eval in x_axis])
...
def interpolation_at_point2(grid_N, t_max, model, init_cond, params_odeint, atol, rtol):
   # return an array with columns - grid_size
    ...
    for i, grid_size in enumerate(grid_N):
        ...
        cheb_grid = np.array([0.5 * t_max - 0.5*t_max* np.cos((2 * i - 1) / (2. * grid_size) * np.pi) for i in range(1, grid_size + 1)])
        func_eval =  discretize_oscillator_odeint(model, init_cond, cheb_grid, params_odeint, atol, rtol)
        output_interpolation[i] = barycentric_interp2(x_eval, cheb_grid, func_eval)
...
def interpolation_at_point(cheb_grids, weights_grid, t_max, model, init_cond, params_odeint, atol, rtol):
   # return an array with columns - grid_size
    ...
    x_eval = 10.0
    i= 0
    for cheb_grid, weights in zip(cheb_grids, weights_grid):
        func_eval =  discretize_oscillator_odeint(model, init_cond, cheb_grid, params_odeint, atol, rtol)
        output_interpolation[i] = barycentric_interp(x_eval, cheb_grid, func_eval, weights)
        i+=1
